...
sleep(3);
// var_dump($_POST);
$recipients = 'contacto@lovestorytravels.com';
// //$recipients = '#';
...
try {
    ...
    $template = file_get_contents('book-now.tpl');

    if (isset($_POST['formType'])) {
        switch ($_POST['formType']){
            case 'reservation':
                $subject = 'Love Story Travels: New Reservation';
                break;
            case 'subscribe':
                $subject = 'Love Story Travels: Subscribe Request';
                break;
    ...
    if (isset($_POST['cName'])) {
        $template = str_replace(
            ...
            array("Name:", $_POST['cName'].$_POST['lastName']),
            $template);
    }
    
    if (isset($_POST['email'])) {
        $template = str_replace(
            array("<!-- #{FromEmail} -->", "<!-- #{FromEmailVal} -->"),
    ...
    if (isset($_POST['dateRange'])) {
        $template = str_replace(
            ...
            array("Date Range:", $_POST['dateRange']),
            $template);
    }
    
    if (isset($_POST['currentHotelName'])) {
        $template = str_replace(
            array("<!-- #{FromHotel} -->", "<!-- #{FromHotelVal} -->"),
            array("Hotel:", $_POST['currentHotelName']),
            $template);
    }
    ...
    if (isset($_POST['extras'])) {
        ...
        $template = str_replace(
            array("<!-- #{FromExtras} -->", "<!-- #{FromExtrasVal} -->"),
            array("Extras:", $extraName),
            $template);
    }
